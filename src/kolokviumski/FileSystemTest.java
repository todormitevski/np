...
class Folder extends File implements IFile{
    ...
    @Override
    public long getFileSize() {
        return files.stream().mapToLong(IFile::getFileSize).sum(); //getFileSize returns long, this is why we use mapToLong
    }
    ...
    public String getFileInfo(int indent) {
        ...
        files.stream().forEach(files -> stringBuilder.append(files.getFileInfo(indent + 1)));
