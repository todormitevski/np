...
class Driver implements Comparable<Driver>{
    ...
    private int lap2;
    private int lap3;
    private int bestLap; //shortest lap
    ...
    public static int stringToTime(String time){
        ...
        return Integer.parseInt(parts[0]) * 60 * 1000 //to secs, then millisecs
                + Integer.parseInt(parts[1]) * 1000 //to millisecs
                + Integer.parseInt(parts[2]);
    }

    public static String timeToString(int time){
        int mins = (time / 1000) / 60;
        int secs = (time - mins * 1000 * 60) / 1000;
        int millisecs = time % 1000;
        return String.format("%d:%02d:%03d", mins, secs, millisecs);
    }
